...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, model_dim: int, num_heads: int):
        ...
        self.attention_layer = self.MultiHeadedSelfAttention(model_dim, num_heads)
        self.feedForward = self.VanillaNeuralNetwork(model_dim)

        self.layer_one = nn.LayerNorm(model_dim)
        self.layer_two = nn.LayerNorm(model_dim)
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        embedded = embedded + self.attention_layer(self.layer_one(embedded))
        embedded = embedded + self.feedForward(self.layer_two(embedded))

        return torch.round(embedded * 10000) / 10000
